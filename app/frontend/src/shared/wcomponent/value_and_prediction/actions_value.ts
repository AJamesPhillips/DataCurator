...
export function get_action_status_of_VAP_set (VAP_set: StateValueAndPredictionsSet): ActionStatusType | undefined
{
    ...
    VAP_set.entries.forEach(entry =>
    {
        if (Math.max(conviction, entry.conviction) !== 1) return
        if (Math.max(probability, entry.probability) !== 1) return
        if (is_valid_action_status(entry.value)) status = entry.value
    })
